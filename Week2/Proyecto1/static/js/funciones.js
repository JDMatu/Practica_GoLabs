...
document.addEventListener('DOMContentLoaded', () => {
    document.getElementById('ingredientes-form').addEventListener('submit', function(event) {
        event.preventDefault();
        let ingredientes = document.getElementById('ingredientes_S').value.split(',').map(ing => ing.trim());
        ...
        .then(data => {
            ...
            if (data.status === "success" && data.recetas.length > 0) {
                data.recetas.forEach(receta => {
                    let recetaDiv = document.createElement('div');
                    recetaDiv.classList.add('col-md-5', 'my-3');
                    recetaDiv.innerHTML = `
                        <div class="card">
                            <div class="card-body">
                                ...
                                <p class="card-text">${receta[2].split('\n').map(ing => `<li>${ing.replace('• ', '')}</li>`).join('')}</p>
                                <button class="btn btn-sm btn-success" data-bs-toggle="modal" data-bs-target="#recetaModal" data-procedimiento="${receta[3]}">Ver Receta</button>
                            </div>
                        </div>
                    `;
    ...
    document.addEventListener('click', function(event) {
        if (event.target.matches('[data-bs-target="#recetaModal"]')) {
            const procedimiento = event.target.getAttribute('data-procedimiento');
            document.getElementById('recetaProcedimiento').innerText = procedimiento;
        }
    });
});
